...
def runProject(stage_name, tc, duration, cnvId, threadCount, delay, rndResCnt){
    ...
    node {
        try{
            timeout(time: timeOut, unit: 'MINUTES') {
                sh "${jmeter_home}/bin/jmeter.sh -n -l ${jmeter_home}/prj/summary-report.csv -t ${jmeter_home}/prj/FCTG-LT-PP.jmx -JRND_RES_CNT=${rndResCnt} -JCNV_ID=${cnvId} -JTESTCASE=${tc} -JTHREADS=${threadCount} -JRAMPUP=${delay} -JDURATION=${duration} -JLOOP_COUNT=1 -JSTARTUP_DELAY=0 -j ${jmeter_home}/prj/jmeter.log"
...
testcaseList.each {
    ...
    threadList.each {
        ...
        delayList.each {
            def delay = it;
            runProject(tc, tc, duration, cnvId, threadCount, delay, rndResCnt)
        }
    }
    sleep interval
}